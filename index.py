...
bot = commands.Bot(command_prefix='발로야 ')

opggsummonersearch = 'https://www.op.gg/summoner/userName='
bottoken = 'Nzk5ODQwMjQ2NzE5MzE1OTg5.YAJbRw.yCzKBq_Qab-yQoxcIR-rZJH3DQE'
...
async def on_message(message): # on_message() event : when the bot has recieved a message
    ...
    if message.author == client.user:
        return
    if message.content.startswith("l!롤전적" or "L!롤전적"):
        try:
            if len(message.content.split(" ")) == 1:
                ...
                embed.set_footer(text='롤 전적 검색 결과')
                ...
                if len(solorank_Point_and_winratio) == 0 and len(flexrank_Point_and_winratio) == 0:
                    ...
                    embed.add_field(name="Search From op.gg", value=opggsummonersearch + playerNickname,
                                    inline=False)
                    embed.add_field(name="Ranked Solo(솔랭) : Unranked", value="Unranked", inline=False)
                    embed.add_field(name="Flex 5:5 Rank(자랭) : Unranked", value="Unranked", inline=False)
                    embed.set_thumbnail(url='https:' + RankMedal[0]['src'])
                    embed.set_footer(text='롤전적 검색')
                ...
                # 자유랭크 기록이 없는경우
                    ...
                    embed.add_field(name="Flex 5:5 Rank(자랭) : Unranked", value="Unranked", inline=False)
                    embed.add_field(name="Most Used Champion(소환사가 자주쓰는 챔프) : " + mostUsedChampion,
                                    value="KDA : " + mostUsedChampionKDA + " / " + "WinRate : " + mostUsedChampionWinRate,
                                    inline=False)
                    embed.set_thumbnail(url='https:' + RankMedal[0]['src'])
                    embed.set_footer(text='롤 전적 검색')
